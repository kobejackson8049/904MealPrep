...
function OrderPage({ orderingOpen, countdown }: { orderingOpen: boolean; countdown: Countdown }) {
  ...
  async function submitOrder() {
    ...
    try {
      ...
      const payload = await response.json().catch(() => null) as { error?: string; order?: { orderNumber: string }; payment?: { instructions: string } } | null;
...
function CheckoutPanel({ step, setStep, draft, updateDraft, items, totalMeals, mealSubtotal, premiumCharges, deliveryFee, total, submitted, submittedOrder, submitting, error, onComplete }: { step: number; setStep: (step: number) => void; draft: OrderDraft; updateDraft: (changes: Partial<OrderDraft>) => void; items: SelectedItem[]; totalMeals: number; mealSubtotal: number; premiumCharges: number; deliveryFee: number; total: number; submitted: boolean; submittedOrder: { orderNumber: string; instructions: string } | null; submitting: boolean; error: string; onComplete: () => void }) {
  ...
  const canContinue = step !== 4 || !manualPayment || Boolean(draft.expectedSenderName.trim());
...
function PaymentStep({ draft, updateDraft, total }: { draft: OrderDraft; updateDraft: (changes: Partial<OrderDraft>) => void; total: number }) {
  const methods: Array<[PaymentMethod, string, string]> = ([['square', 'Square / Apple Pay', 'Preferred checkout. Apple Pay appears when supported by your device.'], ['cash_app', 'Cash App', 'Manual confirmation after payment is sent.'], ['venmo', 'Venmo', 'Manual confirmation after payment is sent.'], ['zelle', 'Zelle', 'Manual confirmation after payment is sent.'], ['other_manual', 'Other supported method', 'Ask us for the current option.']] as Array<[PaymentMethod, string, string]>).filter(([value]) => value === 'square' || value === 'other_manual' || configuredPaymentOptions[value]?.enabled !== false);
  ...
  return <div><StepLabel number="4">Choose payment</StepLabel><div className="mb-5 border border-[#efb22d]/45 bg-[#0b6470] p-4 text-sm"><p className="font-extrabold text-[#efb22d]">Amount due: {money(total)}</p><p className="mt-1 text-[#f7f4eb]/70">Manual payments are not marked paid until the owner verifies receipt.</p></div><div className="space-y-3">{methods.map(([value, title, copy]) => <button key={value} type="button" onClick={() => updateDraft({ paymentMethod: value })} className={`flex w-full items-start justify-between gap-4 border p-4 text-left ${draft.paymentMethod === value ? 'border-[#efb22d] bg-[#0b6470]' : 'border-[#f7f4eb]/20 hover:border-[#f7f4eb]/50'}`}><div><p className="font-extrabold">{title}{value === 'square' && <span className="ml-2 bg-[#efb22d] px-2 py-1 text-[9px] uppercase tracking-[.1em] text-[#073d45]">Preferred</span>}</p><p className="mt-1 text-sm text-[#f7f4eb]/58">{copy}</p></div><span className={`mt-1 grid h-5 w-5 shrink-0 place-items-center rounded-full border ${draft.paymentMethod === value ? 'border-[#efb22d] bg-[#efb22d]' : 'border-[#f7f4eb]/35'}`}>{draft.paymentMethod === value && <Check size={12} className="text-[#073d45]" />}</span></button>)}</div>{manual && <div className="mt-5 border border-[#efb22d]/60 bg-[#0b6470] p-5"><p className="text-sm font-extrabold text-[#efb22d]">{instructions}</p>{option?.qrPath && <img src={option.qrPath} alt={`${paymentLabel(draft.paymentMethod)} payment QR code`} className="mt-4 h-40 w-40 bg-white object-contain p-2" />}<button type="button" onClick={() => void navigator.clipboard.writeText(instructions)} className="mt-3 border border-[#f7f4eb]/30 px-3 py-2 text-[10px] font-extrabold uppercase tracking-[.12em]">Copy payment instructions</button><label className="mt-5 block text-xs font-bold uppercase tracking-[.12em]">Name on the payment account<input value={draft.expectedSenderName} onChange={(event) => updateDraft({ expectedSenderName: event.target.value })} className="mt-2 block w-full border border-[#f7f4eb]/25 bg-[#073d45] px-3 py-3 text-sm font-normal normal-case tracking-normal text-[#f7f4eb] outline-none focus:border-[#efb22d]" placeholder="Exactly as it appears in Cash App, Venmo, or Zelle" required /></label></div>}</div>;
}
