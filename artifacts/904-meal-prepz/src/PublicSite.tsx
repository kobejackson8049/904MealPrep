...
function CheckoutPanel({ step, setStep, draft, updateDraft, items, totalMeals, mealSubtotal, premiumCharges, deliveryFee, total, submitted, submittedOrder, submitting, error, onComplete }: { step: number; setStep: (step: number) => void; draft: OrderDraft; updateDraft: (changes: Partial<OrderDraft>) => void; items: SelectedItem[]; totalMeals: number; mealSubtotal: number; premiumCharges: number; deliveryFee: number; total: number; submitted: boolean; submittedOrder: { orderNumber: string; instructions: string } | null; submitting: boolean; error: string; onComplete: () => void }) {
  ...
  const fulfillmentComplete = draft.fulfillment === 'pickup' ? Boolean(draft.pickupWindow) : Boolean(draft.deliveryZone && draft.address.trim());
  const customerComplete = Boolean(draft.name.trim() && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(draft.email.trim()) && draft.phone.trim());
  const canContinue = step === 2 ? fulfillmentComplete : step === 3 ? customerComplete : step !== 4 || !manualPayment || Boolean(draft.expectedSenderName.trim());
...
function FulfillmentStep({ draft, updateDraft }: { draft: OrderDraft; updateDraft: (changes: Partial<OrderDraft>) => void }) {
  return <div><StepLabel number="2">Pickup or delivery</StepLabel><p className="mb-5 text-sm text-[#f7f4eb]/65">Choose how you will receive your meals. Delivery orders must include the full destination for the kitchen team.</p><div className="grid gap-4 sm:grid-cols-2">{[['pickup', 'Sunday pickup', 'Choose a window and collect from Jacksonville.'], ['delivery', 'Local delivery', 'We deliver across the 904 zone for a small fee.']].map(([value, title, copy]) => <button key={value} type="button" onClick={() => updateDraft({ fulfillment: value as Fulfillment })} className={`border p-5 text-left transition ${draft.fulfillment === value ? 'border-[#efb22d] bg-[#0b6470]' : 'border-[#f7f4eb]/20 hover:border-[#f7f4eb]/50'}`}><div className="flex justify-between gap-3"><span className="text-lg font-extrabold">{title}</span><span className={`grid h-5 w-5 place-items-center rounded-full border ${draft.fulfillment === value ? 'border-[#efb22d] bg-[#efb22d]' : 'border-[#f7f4eb]/35'}`}>{draft.fulfillment === value && <Check size={12} className="text-[#073d45]" />}</span></div><p className="mt-2 text-sm leading-6 text-[#f7f4eb]/60">{copy}</p></button>)}</div>{draft.fulfillment === 'pickup' ? <label className="mt-6 block text-xs font-bold uppercase tracking-[.12em]">Pickup window<select value={draft.pickupWindow} onChange={(event) => updateDraft({ pickupWindow: event.target.value })} className="mt-2 block w-full border border-[#f7f4eb]/25 bg-[#0b6470] px-3 py-3 text-sm font-normal normal-case tracking-normal text-[#f7f4eb] outline-none focus:border-[#efb22d]">{weeklyMenu.pickupWindows.map((window) => <option key={window} value={window}>{window}</option>)}</select></label> : <div className="mt-6 grid gap-4 sm:grid-cols-2"><label className="text-xs font-bold uppercase tracking-[.12em]">Delivery zone<select value={draft.deliveryZone} onChange={(event) => updateDraft({ deliveryZone: event.target.value })} className="mt-2 block w-full border border-[#f7f4eb]/25 bg-[#0b6470] px-3 py-3 text-sm font-normal normal-case tracking-normal text-[#f7f4eb] outline-none focus:border-[#efb22d]">{weeklyMenu.deliveryZones.map((zone) => <option key={zone.id} value={zone.id}>{zone.name} / +{money(zone.fee)}</option>)}</select></label><label className="text-xs font-bold uppercase tracking-[.12em]">Full delivery address<input required value={draft.address} onChange={(event) => updateDraft({ address: event.target.value })} className="mt-2 block w-full border border-[#f7f4eb]/25 bg-[#0b6470] px-3 py-3 text-sm font-normal normal-case tracking-normal text-[#f7f4eb] outline-none placeholder:text-[#f7f4eb]/35 focus:border-[#efb22d]" placeholder="Street, unit, city, state, ZIP" /></label></div>}</div>;
}
...
function PaymentStep({ draft, updateDraft, total }: { draft: OrderDraft; updateDraft: (changes: Partial<OrderDraft>) => void; total: number }) {
  ...
  const squarePaymentLink = (import.meta.env.VITE_SQUARE_PAYMENT_LINK || '').trim();
  ...
  return <div><StepLabel number="4">Choose payment</StepLabel><div className="mb-5 border border-[#efb22d]/45 bg-[#0b6470] p-4 text-sm"><p className="font-extrabold text-[#efb22d]">Amount due: {money(total)}</p><p className="mt-1 text-[#f7f4eb]/70">Manual payments are not marked paid until the owner verifies receipt.</p></div><div className="space-y-3">{methods.map(([value, title, copy]) => <button key={value} type="button" onClick={() => updateDraft({ paymentMethod: value })} className={`flex w-full items-start justify-between gap-4 border p-4 text-left ${draft.paymentMethod === value ? 'border-[#efb22d] bg-[#0b6470]' : 'border-[#f7f4eb]/20 hover:border-[#f7f4eb]/50'}`}><div><p className="font-extrabold">{title}</p><p className="mt-1 text-sm text-[#f7f4eb]/58">{copy}</p></div><span className={`mt-1 grid h-5 w-5 shrink-0 place-items-center rounded-full border ${draft.paymentMethod === value ? 'border-[#efb22d] bg-[#efb22d]' : 'border-[#f7f4eb]/35'}`}>{draft.paymentMethod === value && <Check size={12} className="text-[#073d45]" />}</span></button>)}</div>{!manual && <div className="mt-5 border border-[#efb22d]/60 bg-[#0b6470] p-5"><p className="text-sm font-extrabold text-[#efb22d]">Square secure payment</p><p className="mt-2 text-sm text-[#f7f4eb]/70">Square is the only payment option that opens an external checkout link.</p>{squarePaymentLink ? <a href={squarePaymentLink} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-2 bg-[#efb22d] px-4 py-3 text-xs font-extrabold uppercase tracking-[.12em] text-[#073d45]">Pay {money(total)} with Square <ArrowRight size={15} /></a> : <p className="mt-4 border border-[#f7f4eb]/20 p-3 text-xs text-[#f7f4eb]/65">The owner still needs to add the Square payment-link URL.</p>}</div>}{manual && <div className="mt-5 border border-[#efb22d]/60 bg-[#0b6470] p-5"><p className="text-sm font-extrabold text-[#efb22d]">{paymentLabel(draft.paymentMethod)} payment instructions</p><p className="mt-2 text-base font-bold">{instructions}</p>{option?.qrPath && <img src={option.qrPath} alt={`${paymentLabel(draft.paymentMethod)} payment QR code`} className="mt-4 h-40 w-40 bg-white object-contain p-2" />}<button type="button" onClick={() => void navigator.clipboard.writeText(instructions)} className="mt-3 border border-[#f7f4eb]/30 px-3 py-2 text-[10px] font-extrabold uppercase tracking-[.12em]">Copy payment instructions</button><label className="mt-5 block text-xs font-bold uppercase tracking-[.12em]">Name on the payment account<input value={draft.expectedSenderName} onChange={(event) => updateDraft({ expectedSenderName: event.target.value })} className="mt-2 block w-full border border-[#f7f4eb]/25 bg-[#073d45] px-3 py-3 text-sm font-normal normal-case tracking-normal text-[#f7f4eb] outline-none focus:border-[#efb22d]" placeholder="Exactly as it appears on your payment account" required /></label></div>}</div>;
}
