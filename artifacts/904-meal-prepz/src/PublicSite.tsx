...
type PaymentMethod = 'square' | 'cash_app' | 'venmo' | 'zelle' | 'other_manual';
type OrderDraft = {
...
function paymentLabel(method: PaymentMethod) {
  return ({ square: 'Square / Apple Pay', cash_app: 'Cash App', venmo: 'Venmo', zelle: 'Zelle', other_manual: 'Other supported method' })[method];
}
...
function CustomerStep({ draft, updateDraft }: { draft: OrderDraft; updateDraft: (changes: Partial<OrderDraft>) => void }) {
  ...
  return <div><StepLabel number="3">Your information</StepLabel><div className="grid gap-4 sm:grid-cols-2">{fields.map(([key, label, placeholder]) => <label key={key} className="text-xs font-bold uppercase tracking-[.12em]">{label}<input value={draft[key] as string} onChange={(event) => updateDraft({ [key]: event.target.value })} className="mt-2 block w-full border border-[#f7f4eb]/25 bg-[#0b6470] px-3 py-3 text-sm font-normal normal-case tracking-normal text-[#f7f4eb] outline-none placeholder:text-[#f7f4eb]/35 focus:border-[#efb22d]" placeholder={placeholder} /></label>)}<label className="text-xs font-bold uppercase tracking-[.12em] sm:col-span-2">Notes <textarea value={draft.notes} onChange={(event) => updateDraft({ notes: event.target.value })} className="mt-2 block min-h-28 w-full border border-[#f7f4eb]/25 bg-[#0b6470] px-3 py-3 text-sm font-normal normal-case tracking-normal text-[#f7f4eb] outline-none placeholder:text-[#f7f4eb]/35 focus:border-[#efb22d]" placeholder="Allergies, timing notes, or anything else we should know." /></label></div></div>;
}
function PaymentStep({ draft, updateDraft }: { draft: OrderDraft; updateDraft: (changes: Partial<OrderDraft>) => void }) {
  const methods: Array<[PaymentMethod, string, string]> = [['square', 'Square / Apple Pay', 'Preferred checkout. Apple Pay appears when supported by your device.'], ['cash_app', 'Cash App', 'Manual confirmation after payment is sent.'], ['venmo', 'Venmo', 'Manual confirmation after payment is sent.'], ['zelle', 'Zelle', 'Manual confirmation after payment is sent.'], ['other_manual', 'Other supported method', 'Ask us for the current option.']];
  return <div><StepLabel number="4">Choose payment</StepLabel><div className="mb-5 border border-[#efb22d]/45 bg-[#0b6470] p-4 text-sm"><p className="font-extrabold text-[#efb22d]">Pricing for this week</p><p className="mt-1 text-[#f7f4eb]/70">Standard meals are $8. Premium meals are $10. This preview does not charge a card or wallet.</p></div><div className="space-y-3">{methods.map(([value, title, copy]) => <button key={value} type="button" onClick={() => updateDraft({ paymentMethod: value })} className={`flex w-full items-start justify-between gap-4 border p-4 text-left ${draft.paymentMethod === value ? 'border-[#efb22d] bg-[#0b6470]' : 'border-[#f7f4eb]/20 hover:border-[#f7f4eb]/50'}`}><div><p className="font-extrabold">{title}{value === 'square' && <span className="ml-2 bg-[#efb22d] px-2 py-1 text-[9px] uppercase tracking-[.1em] text-[#073d45]">Preferred</span>}</p><p className="mt-1 text-sm text-[#f7f4eb]/58">{copy}</p>{draft.paymentMethod === value && value !== 'square' && <p className="mt-3 border-t border-[#f7f4eb]/15 pt-3 text-xs font-semibold text-[#efb22d]">{paymentInstructions(value)}</p>}</div><span className={`mt-1 grid h-5 w-5 shrink-0 place-items-center rounded-full border ${draft.paymentMethod === value ? 'border-[#efb22d] bg-[#efb22d]' : 'border-[#f7f4eb]/35'}`}>{draft.paymentMethod === value && <Check size={12} className="text-[#073d45]" />}</span></button>)}</div></div>;
}
...
function Confirmation({ total, method }: { total: number; method: PaymentMethod }) {
  return <div className="mt-12 max-w-[720px] border border-[#5cc2a0]/40 bg-[#0b6470] p-7"><span className="grid h-12 w-12 place-items-center rounded-full bg-[#5cc2a0] text-[#073d45]"><Check size={25} /></span><p className="mt-6 text-[10px] font-extrabold uppercase tracking-[.16em] text-[#efb22d]">Step 6 / order confirmation</p><h3 className="mt-2 text-3xl font-extrabold">Your preorder is in.</h3><p className="mt-4 text-sm leading-6 text-[#f7f4eb]/68">We received your {money(total)} preorder using {paymentLabel(method)}. {method === 'square' ? 'Square checkout is the next handoff when live payments are connected.' : 'The owner will confirm your manual payment and follow up with final pickup or delivery details.'}</p><div className="mt-7 flex flex-wrap gap-3"><Link href="/menu" className="bg-[#efb22d] px-4 py-3 text-xs font-extrabold uppercase tracking-[.12em] text-[#073d45]">Browse the menu</Link><Link href="/" className="border border-[#f7f4eb]/25 px-4 py-3 text-xs font-extrabold uppercase tracking-[.12em]">Back home</Link></div></div>;
}

function HowItWorksPage() {
  const steps = [['01', 'The menu drops', 'A fresh weekly lineup goes live with a clear order cutoff.'], ['02', 'Choose meals', 'Pick your favorites and set quantities directly. No cart gymnastics.'], ['03', 'Select fulfillment', 'Choose a Sunday pickup window or a local delivery zone.'], ['04', 'Pay', 'Square / Apple Pay is preferred, with manual options in the same system.'], ['05', 'We prep', 'Meals are cooked in small batches and organized for the week ahead.'], ['06', 'Heat, eat, repeat', 'Pick up or receive your order, then get on with your week.']];
