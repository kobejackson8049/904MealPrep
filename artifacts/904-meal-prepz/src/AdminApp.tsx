...
export default function AdminApp() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (apiSettings) setSettings({ businessName: apiSettings.businessName, phone: apiSettings.phone, email: apiSettings.email, instagram: apiSettings.instagram, pickupInformation: apiSettings.pickupInformation, announcement: apiSettings.announcement, pricing: { standardPrice: Number(apiSettings.standardPrice), premiumCharge: Number(apiSettings.premiumCharge) }, showDemoLabel: apiSettings.showDemoLabel });
  ...
  async function markPaid(id: string) {
    ...
    setOrders((current) => current.map((order) => order.id === id && order.payment !== 'Refunded' ? { ...order, payment: 'Paid' } : order));
...
function Payments({ orders, onMarkPaid }: { orders: AdminOrder[]; onMarkPaid: (id: string) => void }) {
  ...
  return <div className="space-y-6"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Paid revenue" value={money(paid.reduce((sum, order) => sum + order.total, 0))} detail="All confirmed payment methods" /><Metric label="Pending payments" value={String(pending.length)} detail="Manual confirmation needed" /><Metric label="Amount pending" value={money(pending.reduce((sum, order) => sum + order.total, 0))} detail="Not included in paid revenue" /><Metric label="Refunded payments" value={String(orders.filter((order) => order.payment === 'Refunded').length)} detail="Review in provider" /></div><Panel title="Revenue by payment method" eyebrow="One normalized order ledger"><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">{paymentMethods.map((method) => { const methodOrders = orders.filter((order) => order.paymentMethod === method); const revenue = methodOrders.filter((order) => order.payment === 'Paid').reduce((sum, order) => sum + order.total, 0); const pendingValue = methodOrders.filter((order) => order.payment !== 'Paid' && order.payment !== 'Refunded').reduce((sum, order) => sum + order.total, 0); return <div key={method} className="border border-[#173c3a]/10 bg-white p-4"><p className="text-xs font-extrabold">{method}</p><p className="mt-5 text-2xl font-extrabold">{money(revenue)}</p><p className="mt-1 text-xs text-[#738681]">{methodOrders.length} orders / {money(pendingValue)} pending</p></div>; })}</div></Panel><Panel title="Payment queue" eyebrow="Confirm manual transfers"><div className="overflow-x-auto"><table className="w-full min-w-[700px] text-left text-sm"><thead><tr className="border-b border-[#173c3a]/10 text-[10px] uppercase tracking-[.12em] text-[#738681]"><th className="pb-3">Order</th><th className="pb-3">Customer</th><th className="pb-3">Method</th><th className="pb-3">Amount</th><th className="pb-3">Status</th><th className="pb-3">Action</th></tr></thead><tbody>{pending.map((order) => <tr key={order.id} className="border-b border-[#173c3a]/8 last:border-0"><td className="py-4 font-bold">{order.orderNumber}</td><td className="py-4">{order.customer}</td><td className="py-4">{order.paymentMethod}</td><td className="py-4 font-bold">{money(order.total)}</td><td className="py-4"><StatusPill label={order.payment} kind="yellow" /></td><td className="py-4"><button type="button" onClick={() => onMarkPaid(order.id)} className="text-[10px] font-bold uppercase tracking-[.1em] text-[#27625a] hover:text-[#b28b17]" data-testid={`button-payment-mark-paid-${order.id}`}>Mark paid</button></td></tr>)}</tbody></table></div>{!pending.length && <EmptyState title="Payment queue is clear" copy="All current orders have a confirmed or refunded payment state." />}</Panel></div>;
}

function SettingsPage({ settings, onSave }: { settings: Settings; onSave: (settings: Settings) => void }) {
  const [draft, setDraft] = useState(settings);
  return <Panel title="Business settings" eyebrow="Owner configuration"><div className="mb-6 border border-[#e7d58d] bg-[#fff9df] p-4 text-sm text-[#7b6414]"><Settings2 size={17} className="mb-2" /><strong>Provider credentials stay server-side.</strong><p className="mt-1 text-xs">Contact and pricing fields are safe to edit here; payment credentials and secrets are never placed in the client.</p></div><div className="grid gap-4 sm:grid-cols-2">{[['Business name', 'businessName'], ['Phone', 'phone'], ['Email', 'email'], ['Instagram', 'instagram'], ['Pickup information', 'pickupInformation']].map(([label, key]) => <label key={key} className="text-xs font-bold uppercase tracking-[.12em]">{label}<input value={draft[key as keyof Settings] as string} onChange={(event) => setDraft({ ...draft, [key]: event.target.value })} className="mt-2 block w-full border border-[#173c3a]/15 bg-white px-3 py-3 text-sm font-normal normal-case tracking-normal outline-none focus:border-[#27625a]" /></label>)}<label className="text-xs font-bold uppercase tracking-[.12em]">Customer announcement<textarea value={draft.announcement} onChange={(event) => setDraft({ ...draft, announcement: event.target.value })} className="mt-2 block min-h-24 w-full border border-[#173c3a]/15 bg-white px-3 py-3 text-sm font-normal normal-case tracking-normal outline-none focus:border-[#27625a]" /></label></div><div className="mt-8 border-t border-[#173c3a]/10 pt-6"><h3 className="text-sm font-extrabold">Automatic meal pricing</h3><p className="mt-1 text-xs text-[#738681]">Meals use the standard base price. Premium meals add the configured surcharge automatically.</p><div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="text-xs font-bold uppercase tracking-[.12em]">Standard price<input type="number" min="0" step="0.01" value={draft.pricing.standardPrice} onChange={(event) => setDraft({ ...draft, pricing: { ...draft.pricing, standardPrice: Number(event.target.value) } })} className="mt-2 block w-full border border-[#173c3a]/15 bg-white px-3 py-3 text-sm font-normal normal-case tracking-normal" /></label><label className="text-xs font-bold uppercase tracking-[.12em]">Premium surcharge<input type="number" min="0" step="0.01" value={draft.pricing.premiumCharge} onChange={(event) => setDraft({ ...draft, pricing: { ...draft.pricing, premiumCharge: Number(event.target.value) } })} className="mt-2 block w-full border border-[#173c3a]/15 bg-white px-3 py-3 text-sm font-normal normal-case tracking-normal" /></label></div></div><label className="mt-6 flex items-center gap-3 text-sm font-semibold"><input type="checkbox" checked={draft.showDemoLabel} onChange={(event) => setDraft({ ...draft, showDemoLabel: event.target.checked })} /> Show “demo workspace” messaging</label><button type="button" onClick={() => onSave(draft)} className="mt-7 bg-[#173c3a] px-5 py-3 text-xs font-bold uppercase tracking-[.12em] text-white hover:bg-[#27625a]" data-testid="button-save-settings">Save settings</button></Panel>;
}
